...
/**
 * Here we show uploading a large file in a background thread, trying to show typical
...
public class LargeFileUpload extends AsyncTask<Void, Long, Boolean> {
	...
	public LargeFileUpload(Context context, VDiskAPI<?> api, String VDiskPath,
		...
		mDialog.setMax(100);
		mDialog.setMessage("正在上传：" + file.getName());
		mDialog.setProgressStyle(ProgressDialog.STYLE_HORIZONTAL);
		mDialog.setProgress(0);
		mDialog.setButton("取消", new OnClickListener() {
	...
	protected Boolean doInBackground(Void... params) {
		try {
			...
			mApi.putLargeFileOverwriteRequest(mSrcPath, desPath, mFile.length(), handler);
